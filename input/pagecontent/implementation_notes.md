...
§NR1?^admin-service:When the Consent record is created on System A, System A's system identifier **SHALL** be recorded in the `manager` extension of the Consent instance.§ This originator identifier denotes to all recipients of the shared Consent what system originated it. §NR2?^client,admin-service:Systems that ingest a Consent from another system **SHALL** preserve the `manager` extension value unchanged.§ Thus, when the Consent resource is shared with another system or systems, a relationship between the originating system and the recipient system(s) is established.
