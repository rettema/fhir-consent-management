...
Implementers should design their systems with this dependency in mind: §IN2?^client:a system that cannot maintain a subscription to the consent management source **SHOULD** fetch a fresh copy of the consent at the time of each authorization decision rather than rely on a cached copy.§
...
§NR1?^consent-server:When the Consent record is created on System A, System A's system identifier **SHALL** be recorded in the `manager` extension of the Consent instance.§ This originator identifier denotes to all recipients of the shared Consent what system originated it. §NR2?^client,consent-server:Systems that ingest a Consent from another system **SHALL** preserve the `manager` extension value unchanged.§ Thus, when the Consent resource is shared with another system or systems, a relationship between the originating system and the recipient system(s) is established.
