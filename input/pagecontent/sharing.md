...
§NR3?^client,admin-service:When sharing a Consent resource with another system, the originating or forwarding system **SHALL NOT** modify the `manager` extension value.§ The `manager` extension identifies the system that originally created the Consent and must remain unchanged throughout the consent's lifecycle across all sharing relationships.

§NR4?^client,admin-service:A system that receives a shared Consent resource **SHOULD** establish a FHIR Subscription with the system identified in the `manager` extension to receive updates to that Consent (e.g., revocations).§ This ensures that shared copies of the consent remain current.
